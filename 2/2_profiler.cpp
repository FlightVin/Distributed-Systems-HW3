...
vector<double> read_file(int argc, char* argv[]) {
    ...
    vector<double> result = {static_cast<double>(N), static_cast<double>(M), static_cast<double>(k), c_r, c_i};
    return result;
}
...
void attach_grid_coordinates(double* real_pointer, double* imag_pointer, int i, int j, int N, int M) {
    *real_pointer = -1.5 + (static_cast<double>(i - 1) * 3.0) / static_cast<double>(M - 1);
    *imag_pointer = 1.5 - (static_cast<double>(j - 1) * 3.0) / static_cast<double>(N - 1);
}
...
int main(int argc, char* argv[]) {
    ...
    int N, M, K;
    double c_r, c_i;
    N = static_cast<int>(input_args[0]);
    M = static_cast<int>(input_args[1]);
    K = static_cast<int>(input_args[2]);
    c_r = input_args[3];
    c_i = input_args[4];
    ...
    // if (my_rank == 0) {
    //     cout.write(all_strings.data(), total_length);
    //     cout.flush();
    // }
    // **********************************************************************
